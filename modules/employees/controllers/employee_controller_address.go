...
func (ctrl *EmployeeController) addAddress(c *fiber.Ctx) error {
	...
	err = ctrl.addressService.Create(c.Context(), request)
	if err != nil {
...
func (ctrl *EmployeeController) editAddressForm(c *fiber.Ctx) error {
	...
	employeeAddress, err := ctrl.addressService.GetByUniqueId(c.Context(), addId)
	if err != nil {
		return ctrl.HandleErrorsWeb(c, err)
	}
...
func (ctrl *EmployeeController) editAddress(c *fiber.Ctx) error {
	...
	address, err := ctrl.addressService.GetByUniqueId(c.Context(), addId)
	if err != nil {
		return ctrl.HandleErrorsWeb(c, err)
	}
	...
	err = ctrl.addressService.Update(c.Context(), address.AddressId, request)
	if err != nil {
		ctrl.errorLogger.Error(c, err.Error())
		return ctrl.HandleErrorsWeb(c, err)
	}
