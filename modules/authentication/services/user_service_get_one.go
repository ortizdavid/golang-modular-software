...
func (s *UserService) GetUserById(ctx context.Context, userId int64) (entities.UserData, error) {
	...
	if err != nil {
		return entities.UserData{}, apperrors.NotFoundError("user not found. invalid id")
	}
	if user.UserId == 0 {
		return entities.UserData{}, apperrors.NotFoundError("user not found. invalid id")
	}
...
func (s *UserService) GetUserByUniqueId(ctx context.Context, uniqueId string) (entities.UserData, error) {
	...
	if err != nil {
		return entities.UserData{}, apperrors.NotFoundError("user not found. invalid unique id")
	}
	if user.UserId == 0 {
		return entities.UserData{}, apperrors.NotFoundError("user not found. invalid unique id")
	}
...
func (s *UserService) GetUserByName(ctx context.Context, email string) (entities.UserData, error) {
	...
	if err != nil {
		return entities.UserData{}, apperrors.NotFoundError("user not found")
	}
	if user.UserId == 0 {
		return entities.UserData{}, apperrors.NotFoundError("user not found")
	}
...
func (s *UserService) GetUserApiKey(ctx context.Context, xUserId string) (entities.UserApiKey, error) {
	...
	if err != nil {
		return entities.UserApiKey{}, apperrors.NotFoundError("user api key not found. invalid x-user-id")
	}
	if user.UserId == 0 {
		return entities.UserApiKey{}, apperrors.NotFoundError("user api key not found. invalid x-user-id")
	}
...
func (s *UserService) GetUserApiKeyById(ctx context.Context, userId int64) (entities.UserApiKey, error) {
	...
	if err != nil {
		return entities.UserApiKey{}, apperrors.NotFoundError("user api key not found")
	}
	if user.UserId == 0 {
		return entities.UserApiKey{}, apperrors.NotFoundError("user api key not found")
	}
...
func (s *UserService) GetUserApiKeyByName(ctx context.Context, userName string) (entities.UserApiKeyData, error) {
	...
	if err != nil {
		return entities.UserApiKeyData{}, apperrors.NotFoundError("user api key not found")
	}
	if user.UserId == 0 {
		return entities.UserApiKeyData{}, apperrors.NotFoundError("user api key not found")
	}
...
func (s *UserService) GetUserByToken(ctx context.Context, token string) (entities.UserData, error) {
	...
	if err != nil {
		return entities.UserData{}, apperrors.NotFoundError("user not found. invalid token")
	}
	if user.UserId == 0 {
		return entities.UserData{}, apperrors.NotFoundError("user not found. invalid token")
	}
...
func (s *UserService) FindUserByToken(ctx context.Context, token string) (entities.User, error) {
	...
	if err != nil {
		return entities.User{}, apperrors.NotFoundError("user not found. invalid token")
	}
	if user.UserId == 0 {
		return entities.User{}, apperrors.NotFoundError("user not found. invalid token")
	}
...
func (s *UserService) FindUserByEmail(ctx context.Context, email string) (entities.User, error) {
	...
	if err != nil {
		return entities.User{}, apperrors.NotFoundError("user not found. invalid email")
	}
	if user.UserId == 0 {
		return entities.User{}, apperrors.NotFoundError("user not found. invalid email")
	}
